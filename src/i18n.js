// Tiny two-language dictionary. UI strings are authored in Bangla; `t()` maps
// them to English when the user flips the language button. Composite labels
// ("মস · সবুজ") are translated part by part.

const EN = {
  "পানির পরিমাণ": "Water amount",
  "পানি কমাও": "Less water",
  "পানি বাড়াও": "More water",
  "কাঠের বেস ফুলদানি": "Wood-base vase",
  "নাশপাতি কর্ক জার": "Pear cork jar",
  "গোল কর্ক জার": "Cork-ball jar",
  "ছোট ঝামাপাথর": "Tiny pumice",
  "গোল ছোট নুড়ি": "Button pebble",
  // tabs
  "ভাস্কর্য": "Sculpting",
  "পেইন্টিং": "Painting",
  "সাজানো": "Decorations",
  "দৃশ্য": "Scene",
  // tools
  "বসাও": "Place",
  "পানি": "Water",
  "স্প্রে": "Spray",
  "উঁচু": "Raise",
  "নিচু": "Lower",
  "সমান": "Flatten",
  "ঘাস": "Grass",
  "মস ব্রাশ": "Moss brush",
  "নুড়িপথ": "Pebble path",
  "আইটেমের উপরে বসাও": "Stack on items",
  "নদীর নুড়ি": "River pebble",
  "ধাপের পাথর": "Stepping stone",
  "পাতলা স্লেট": "Slate chip",
  "গ্রানাইট": "Granite",
  "লাভা পাথর": "Lava rock",
  "বেলেপাথর": "Sandstone",
  "পাথর, কাঠ বা অন্য আইটেমের উপরে বসাতে চালু করো।": "Turn on to place on rocks, wood or other items.",
  "এই জায়গায় আইটেমের জন্য যথেষ্ট জায়গা নেই।": "There is not enough room for this item here.",
  // categories
  "পছন্দের": "Favorites",
  "জার": "Jars",
  "বেস স্তর": "Base layers",
  "গাছপালা": "Plants",
  "মাশরুম": "Mushrooms",
  "পাথর ও ক্রিস্টাল": "Rocks & crystals",
  "প্রাণী": "Animals",
  "স্থাপনা": "Structures",
  "কাঠ": "Wood",
  "আলো": "Lights",
  // scene panel
  "পরিবেশ": "Scene",
  "বিশ্ব": "World",
  "সম্পাদনা": "Edit",
  "সংগ্রহ": "Library",
  "সমাজ": "Social",
  "আরও": "More",
  "আরও টুল": "More tools",
  "বানানো": "Building",
  "বিল্ডিং": "Building",
  "সেটিংস": "Settings",
  "ঋতু ও আবহাওয়া": "Season & weather",
  "ব্যাকড্রপ শান্ত": "Backdrop calm",
  "টেবিল": "Table",
  "থিম বেছে নাও": "Choose a theme",
  "ক্যামেরা": "Camera",
  "ফোকাস": "Focus",
  "ফোকাস থেকে বের হও": "Exit focus",
  "ফোকাস বিল্ড": "Focus Build",
  "ফোকাস বিল্ড ছাড়ো": "Exit Focus Build",
  "আরাম": "Comfort",
  "ফোকাস মোড": "Focus mode",
  "দ্রুত কাজ": "Quick actions",
  "বের হও": "Exit",
  // Focus HUD
  "মাঝে আনো": "Center camera",
  "ক্যামেরা লক": "Lock camera",
  "ক্যামেরা খোলো": "Unlock camera",
  "ট্রে খোলো": "Open tray",
  "ট্রে বন্ধ করো": "Close tray",
  "ট্রে": "Tray",
  "ক্যামেরা লক — জার আর ঘুরবে না।": "Camera locked — the jar will not turn.",
  "ক্যামেরা খোলা — টেনে ঘোরাও।": "Camera unlocked — drag to turn the jar.",
  "এখন করো": "Do this",
  "আরাম সেটিংস": "Comfort settings",
  "দৃশ্যকে তোমার মতো শান্ত করো": "Make the scene feel calm and personal.",
  "শান্ত UI": "Soft UI",
  "কম অ্যানিমেশন": "Reduced motion",
  "পারফরম্যান্স": "Performance",
  "ছবির মান": "Picture quality",
  "টেনে চওড়া করো": "Drag to resize",
  "প্যানেলের চওড়া": "Panel width",
  "ক্যামেরা রিসেট": "Reset camera",
  "ক্যামেরা আবার আগের জায়গায়।": "Camera back where it started.",
  "সরাও": "Move",
  "নকল করো": "Duplicate",
  "আইটেমের কাজ": "Item actions",
  "কোথায় বসাতে চাও সেখানে ট্যাপ করো": "Tap where you want it",
  "নকল করা হলো": "Duplicated",
  "জার ভরে গেছে — নকল করা গেল না।": "This garden is full \u2014 nothing to duplicate into.",
  "ছেড়ে দিলে বসে যাবে": "Release to place",
  "এখানে বসবে না": "Can\u2019t place here",
  "জারের ভেতরে নিয়ে এসো": "Move inside the jar",
  "সামনে থেকে": "Front view",
  "পাশ থেকে": "Side view",
  "উপর থেকে": "Top view",
  "তিন-চতুর্থাংশ": "Three-quarter",
  "কোণ": "Angle",
  "পাতলা করো": "Thinner",
  "মোটা করো": "Thicker",
  "ক্লাসিক স্তর": "Classic stack",
  "মিমি": "mm",
  "বাকি": "left",
  "চারকোল ফিল্টার — পাতলা হলেই চলে": "Charcoal is a filter \u2014 thin is enough",
  "ড্রেনেজ আরও গভীর হলে ভালো": "Drainage works better deeper",
  "এই স্তরের জন্য বেশি পাতলা": "Thinner than this material usually goes",
  "এই স্তরের জন্য বেশি মোটা": "Thicker than this material usually goes",
  "জারে আর জায়গা নেই": "No room left in the jar",
  // depth, capacity and the layer editor
  "স্তরের গভীরতা": "Layer depth",
  "০ মিমি স্তর হয় না": "0 mm is not a layer",
  "০ মিমি স্তর হয় না — একটু গভীরতা দাও": "0 mm is not a layer \u2014 give it some depth",
  "মোট": "Total",
  "সর্বোচ্চ": "Max",
  // not the tray category "জার" (Jars) — this one names the vessel's capacity
  "জারের ধারণক্ষমতা": "Jar holds",
  "স্তর সাজাও": "Edit layers",
  "স্তরের তালিকা": "Layer list",
  "স্তরের কাজ": "Layer actions",
  "গভীরতা": "Depth",
  "উপরে": "Up",
  "নিচে": "Down",
  "এটাই সবার উপরে": "That is already the top layer",
  "এটাই সবার নিচে": "That is already the bottom layer",
  "এখনো কোনো স্তর নেই — ট্রে থেকে বেস ঢালো।":
    "No layers yet \u2014 pour a base from the tray.",
  // item size
  "সাইজ প্রিসেট": "Size presets",
  // "ছোট" and "বড়" are already in this table further down, shared with the
  // brush-size labels — same words, same translations.
  "স্বাভাবিক": "Default",
  "স্পষ্ট": "Clear",
  "নরম": "Soft",
  "শুধু পেছনের দৃশ্য বদলায়, জারের ভেতরে কিছু নয়।":
    "Changes the background only \u2014 never the jar or what is in it.",
  "বসানোর মোড": "Place mode",
  "নির্বাচনের মোড": "Select mode",
  "সরানোর মোড": "Move mode",
  "বেস মোড": "Base mode",
  "পানির মোড": "Water mode",
  "স্প্রে মোড": "Mist mode",
  "ভাস্কর্য মোড": "Sculpt mode",
  "পেইন্ট মোড": "Paint mode",
  "ট্যাপ বা টেনে মাটিতে পানি দাও": "Tap or drag to water the soil",
  "কাঁচে স্প্রে করতে ট্যাপ করো": "Tap to mist the glass",
  "মাটি উঁচু করতে টেনে নাও": "Drag to raise the ground",
  "মাটি নিচু করতে টেনে নাও": "Drag to lower the ground",
  "মাটি সমান করতে টেনে নাও": "Drag to flatten the ground",
  "ঘাস আঁকতে টেনে নাও": "Drag to paint grass",
  "মস আঁকতে টেনে নাও": "Drag to paint moss",
  "নুড়ি ছড়াতে টেনে নাও": "Drag to scatter pebbles",
  "ট্যাপ করে স্তর দাও": "Tap to add a layer",
  "ট্যাপ করে বসাও": "Tap inside to place",
  "জিনিস বেছে নিতে ট্যাপ করো, সরাতে টেনে নাও": "Tap to select, drag to move",
  "তুলতে টেনে নাও, মেনুর জন্য ট্যাপ করো": "Drag to lift, tap for the menu",
  "তোমার টেরারিয়াম তৈরি হচ্ছে…": "Preparing your terrarium\u2026",
  "দৃশ্য আনা হচ্ছে…": "Loading the scene\u2026",
  "নতুন জার বসানো হচ্ছে…": "Setting up the new jar\u2026",
  "মসৃণ চলা চালু — ছায়া আর কণা কমানো হলো।": "Smooth performance on \u2014 shadows and particles eased back.",
  "ভালো ছবি চালু — পুরো ছায়া আর কণা ফিরে এলো।": "High quality on \u2014 full shadows and particles are back.",
  "ভালো ছবি": "High quality",
  "মসৃণ চলা": "Smooth performance",
  "দুর্বল ডিভাইসে ছায়া আর কণা কমিয়ে দেয় — বসানো আর টানা একই রকম মসৃণ থাকে।": "Eases off shadows and particles on weaker devices \u2014 placing and dragging stay just as smooth.",
  "বোতামের ক্লিক": "Button click sound",
  "UI স্বচ্ছতা": "UI opacity",
  "কম স্বচ্ছতা": "Reduce transparency",
  "কাছে আনো": "Zoom in",
  "অ্যাকাউন্ট খোলো": "Create account",
  "অতিথি হিসেবে চালিয়ে যাও": "Continue as guest",
  "তোমার এখনকার বাগান যেমন আছে তেমনই থাকবে।": "Your current garden stays exactly as it is.",
  "একসঙ্গে বানাতে আর সব ডিভাইসে তোমার ঘর রাখতে অ্যাকাউন্ট খোলো।": "Create an account to build together and keep your room across devices.",
  "কো-অপ ঘর": "Co-op room",
  "একসঙ্গে একটি বাগান বানাতে ডাকা হয়েছে।": "You have been invited to build a garden together.",
  "আমন্ত্রণ লিঙ্ক": "Invite link",
  "আমন্ত্রণ লিঙ্ক কপি": "Copy invite link",
  "নতুন ঘর বানাও": "Create a room",
  "রুম কোড": "Room code",
  "কোড কপি": "Copy code",
  "ঘর ছাড়ো": "Leave room",
  "সঙ্গী": "Partner",
  "তুমি": "You",
  "অথবা": "or",
  "অতিথি": "Guest",
  "পাসওয়ার্ড দেখাও": "Show password",
  "নাম": "Name",
  "ইমেইল": "Email",
  "পাসওয়ার্ড": "Password",
  "অ্যাকাউন্ট": "Account",
  "প্রোফাইল": "Profile",
  "দূরে নাও": "Zoom out",
  "জুম": "Zoom",
  "বেশি কনট্রাস্ট": "Higher contrast",
  "লেখার আকার": "Text size",
  "নড়াচড়া": "Motion",
  "দেখা": "Seeing",
  "সরাসরি সরঞ্জামে যাও": "Skip to the tools",
  "তোমার টেরারিয়াম": "Your terrarium",
  "বন্ধ করো": "Close",
  "ক্যামেরা শান্ত করো": "Center camera",
  "নির্বাচিত আইটেম": "Selected item",
  "গাছ": "Plant",
  "সাজাও": "Decorate",
  "অন্যান্য": "Other",
  "খোঁজো…": "Search…",
  // navbar text actions
  "ফেরাও": "Undo",
  "আবার": "Redo",
  "সদ্য ব্যবহৃত": "Recent",
  "সব": "All",
  "শেষ": "Done",
  "ছবি সেভ করো": "Save this photo",
  "প্রথম স্তর ঢালতে জারে ট্যাপ করো": "Tap the jar to pour your first layer",
  "রিসেট": "Reset",
  "গ্যালারি": "Gallery",
  "থিম": "Themes",
  "গাইডেড অবজেক্টিভ": "Guided objective",
  "আজকের চ্যালেঞ্জ": "Today's challenge",
  "↺ শেষ সেভ ফেরাও": "↺ Restore last save",
  "অটোসেভ চালু": "Autosave on",
  "অর্জন": "Achievements",
  "কো-অপ": "Co-op",
  "কমিউনিটি": "Community",
  "এক্সপ্লোর": "Explore",
  "আমার জার": "My jars",
  "অ্যাকাউন্ট": "Account",
  "আজকের কমিউনিটি চ্যালেঞ্জ": "Today's community challenge",
  "জনপ্রিয় টেরারিয়াম": "Popular terrariums",
  "ক্লাউডে প্রকাশ করো": "Publish to the cloud",
  "তোমার বর্তমান বাগান অন্যদের সঙ্গে শেয়ার করো।": "Share your current garden with other players.",
  "প্রকাশ": "Publish",
  "আমার প্রকাশনা": "My publications",
  "সাইন ইন": "Sign in",
  "অ্যাকাউন্ট খোলো": "Create account",
  "স্বাগতম": "Welcome",
  "সাইন আউট": "Sign out",
  "টেরারিয়াম প্রকাশ": "Publish terrarium",
  "পাবলিক গ্যালারিতে দেখাও": "Show in public gallery",
  "ক্লাউডে সেভ করো": "Save to cloud",
  "সেভ": "Save",
  "জার সাজাও": "Customize jar",
  "শব্দ": "Sound",
  "ইন্ট্রো": "Intro",
  "জার ঘোরাতে টেনে ধরো — নিচের তাক থেকে জিনিস বেছে নাও।":
    "Drag to spin the jar — pick items from the shelf below.",
  "ছবি": "Photo",
  // background (mood) labels
  "স্টুডিও": "Studio",
  "দিন": "Day",
  "গোধূলি": "Dusk",
  "রাত": "Night",
  "লাইব্রেরি": "Library",
  "বাগান": "Garden",
  "সৈকত": "Beach",
  "মহাকাশ": "Space",
  "পাহাড়": "Mountain",
  "বৃষ্টি": "Rain",
  // 3D room backgrounds
  "থ্রিডি": "3D",
  "কাফে": "Café",
  "আর্ট গ্যালারি": "Art gallery",
  "ডাইনিং": "Dining",
  "আর্মরি": "Armory",
  // jars
  "কর্ক জার": "Cork jar",
  "সোজা কর্ক জার": "Cork cylinder",
  "চওড়া কর্ক জার": "Wide cork jar",
  "সরু কর্ক জার": "Tall cork jar",
  "ঢালু কর্ক জার": "Tapered cork jar",
  "গোল কাঁধের কর্ক জার": "Round-shoulder cork jar",
  "বেল জার": "Bell jar",
  "বোতল": "Bottle",
  "ম্যাসন": "Mason jar",
  "গোল জার": "Globe",
  "লম্বা": "Tall cylinder",
  "বাটি": "Bowl",
  "ফ্লাস্ক": "Flask",
  "ডিম জার": "Egg jar",
  "গ্রিনহাউস": "Greenhouse",
  "অষ্টভুজ ঘর": "Octagon house",
  "কাত চূড়া": "Slant tower",
  "ঘুড়ি জার": "Kite jar",
  "ষড়ভুজ রত্ন": "Hex gem",
  "কাঠের বাক্স": "Wood case",
  "ছোট": "Small",
  "বড়": "Large",
  // base layers
  "লেকা বল": "LECA balls",
  "নুড়ি": "Pebbles",
  "স্ফ্যাগনাম": "Sphagnum",
  "চারকোল": "Charcoal",
  "মাটি": "Soil",
  "বালু": "Sand",
  "সাদা বালু": "White sand",
  "প্রবাল লাল বালু": "Coral sand",
  "কমলা বালু": "Amber sand",
  "সোনালি বালু": "Golden sand",
  "সবুজ বালু": "Emerald sand",
  "ফিরোজা বালু": "Turquoise sand",
  "বেগুনি বালু": "Violet sand",
  "রঙিন স্তর": "Rainbow stack",
  "গাঢ় বনমাটি": "Dark forest soil",
  "কচি সবুজ বালু": "Lime sand",
  "হালকা গোলাপি বালু": "Blush sand",
  "কালো কুচি পাথর": "Black gravel",
  "মধুরঙা কুচি পাথর": "Honey gravel",
  "হাতির দাঁত রঙের কুচি": "Ivory gravel",
  "কমলা কুচি পাথর": "Rust gravel",
  "ছবির মতো স্তর": "Photo-inspired layers",
  "বাড়তি দানা": "Extra grain",
  "নির্বাচিত স্তর": "Selected layer",
  "নির্বাচিত স্তরের বাড়তি দানা": "Extra grain for selected layer",
  "পরের স্তরের বাড়তি দানা": "Extra grain for next layer",
  "দানা কমাও": "Less grain",
  "দানা বাড়াও": "More grain",
  "দানার রং": "Grain colour",
  "মিলিয়ে রং": "Matching",
  "প্রাকৃতিক মিশ্রণ": "Natural mix",
  "বর্তমান পরিষ্কার চেহারা": "Current clean look",
  "০% রাখলে বর্তমান পরিষ্কার চেহারা থাকে।": "0% keeps the current clean look.",
  "বাগান সাজিয়ে দাও": "Plant a reference garden",
  "আগে বেস স্তর দাও": "Add base layers first.",
  "এই সাজানোর জন্য খালি বেস দরকার": "This starter needs unplanted layers. Your existing decorations are unchanged.",
  "প্রতিটি গাছ আলাদা করে সরাতে পারো": "Garden planted. Move or resize each item separately; Undo removes the starter.",
  "সবুজ জোড়া দাগ": "Lime twin stripes",
  "গোলাপি জোড়া দাগ": "Blush twin stripes",
  "লাল জোড়া দাগ": "Red twin stripes",
  "বনমাটির স্তর": "Woodland layers",
  "ফিটোনিয়া ঝোপ": "Fittonia bush",
  "আরালিয়া": "Aralia",
  "খাঁজকাটা পাথর": "Crag rock",
  "আলগা কুচি পাথর": "Loose gravel patch",
  // decoration base names
  "মস": "Moss",
  "স্নেক প্ল্যান্ট": "Snake plant",
  "বনসাই": "Bonsai",
  "মস বল": "Moss ball",
  "পাতাগাছ": "Leafy plant",
  "ফার্ন": "Fern",
  "ফিটোনিয়া": "Fittonia",
  "সাকুলেন্ট": "Succulent",
  "এয়ার প্ল্যান্ট": "Air plant",
  "ক্রিস্টাল": "Crystal",
  "পাথর": "Stone",
  "শামুক": "Snail",
  "স্লেট": "Slate",
  "মরচে": "Rusty",
  "ক্যাকটাস": "Cactus",
  "ফুল": "Flowers",
  "সেতু": "Bridge",
  "ছোট্ট ঘর": "Tiny house",
  "লণ্ঠন": "Lantern",
  "প্রজাপতি": "Butterfly",
  "লেডিবাগ": "Ladybug",
  "হরিণ": "Deer",
  "পাইলিয়া": "Pilea",
  "পথোস": "Pothos",
  "ক্যালাথিয়া": "Calathea",
  "ভেনাস ফ্লাইট্র্যাপ": "Venus flytrap",
  "ব্যাঙ": "Frog",
  "কচ্ছপ": "Turtle",
  "পাখি": "Bird",
  "বামন": "Gnome",
  "তোরি গেট": "Torii gate",
  "প্যাগোডা": "Pagoda",
  "বেড়া": "Fence",
  "কুয়া": "Well",
  "জিওড": "Geode",
  "পাইনকোন": "Pinecone",
  "গুঁড়ি": "Log",
  "জলাশয়": "Pond",
  "সাগুয়ারো": "Saguaro",
  "প্রিকলি পিয়ার": "Prickly pear",
  "পিনকুশন": "Pincushion",
  "ক্লিপ লাইট": "Clip light",
  "স্ট্রিপ লাইট": "Strip light",
  "কাঠের ল্যাম্প": "Wood lamp",
  "রিং লাইট": "Ring light",
  // variant suffixes
  "সবুজ": "Green",
  "হলদে": "Yellowish",
  "গাঢ়": "Dark",
  "নীলচে": "Bluish",
  "হালকা": "Light",
  "লালচে": "Reddish",
  "গোলাপি": "Pink",
  "লাল": "Red",
  "সাদা": "White",
  "নীল": "Blue",
  "বেগুনি": "Purple",
  "রুপালি": "Silvery",
  "কমলা": "Orange",
  "বাদামি": "Brown",
  "ফ্যাকাশে": "Pale",
  "স্বচ্ছ": "Clear",
  "বেলে": "Sandy",
  "ধূসর": "Gray",
  "কালো": "Black",
  "ফুলসহ": "Blooming",
  "হলুদ ফুল": "Yellow bloom",
  "ফুলহীন": "No bloom",
  "হলুদ": "Yellow",
  "উষ্ণ": "Warm",
  "শীতল": "Cool",
  "গোল্ডেন": "Golden",
  "শরৎ": "Autumn",
  "দিবা": "Daylight",
  "গ্রো": "Grow",
  "গোলাপি ফুল": "Pink bloom",
  // hints
  "বেস উপাদান বেছে ট্যাপ করো (গোল স্তর) বা ড্র্যাগ করে ইচ্ছেমতো আকৃতিতে মাটি আঁকো।":
    "Pick a base material, then tap for a full layer — or drag to paint substrate in any shape you like.",
  "এবার স্ফ্যাগনাম মসের পাতলা স্তর দাও — এটা মাটিকে নিচের ড্রেনেজে মিশে যাওয়া থেকে আটকায়।":
    "Now add a thin sphagnum moss barrier — it keeps soil out of the drainage below.",
  "এক চিমটি চারকোল ছড়াও — বন্ধ জারের ভেতরটা তাজা রাখে।":
    "Sprinkle a little charcoal — it keeps a closed jar fresh.",
  "এখন মূল স্তর — মাটি। গাছের শিকড়ের জন্য একটু পুরু করে দাও।":
    "Now the main layer — soil. Make it thick enough for roots.",
  "সুন্দর! এবার সাজানোর জিনিস বেছে জারের ভেতরে ট্যাপ করে বসাও।":
    "Lovely! Now pick decorations and tap inside the jar to place them.",
  "দারুণ! ⛰️ টুলে মাটি ভাস্কর্য করো, 🌱 টুলে ঘাস আঁকো, জিনিস ধরে টেনে সাজাও।":
    "Great! Sculpt the land with ⛰️, paint grass with 🌱, and drag items to arrange them.",
  "জার প্রায় ভরে গেছে — এবার সাজানো শুরু করো!":
    "The jar is nearly full — time to decorate!",
  "আগে অন্তত একটা বেস স্তর দাও, তারপর গাছ বসাও।":
    "Add at least one base layer first, then plant.",
  "আগে বেস স্তর দাও, তারপর ভাস্কর্য বা ঘাস।":
    "Lay a base layer first, then sculpt or paint.",
  "ছবি সেভ হয়ে গেছে! বন্ধুদের দেখাও।": "Photo saved! Show your friends.",
  "পছন্দ": "Favorite",
  "জার ছাড়া": "No jar",
  "আমার টেরারিয়াম": "My Terrariums",
  "লোড": "Load",
  "টেরারিয়াম সংরক্ষিত!": "Terrarium saved!",
  "জায়গা নেই — গ্যালারি থেকে কিছু মুছে ফেলো।": "Out of space — delete some from the gallery.",
  "গ্যালারি খালি — 💾 দিয়ে সংরক্ষণ করো।": "Gallery is empty — save with 💾.",
  "পিরামিড": "Pyramid",
  "জিওডেসিক": "Geodesic",
  "রত্ন": "Gem",
  "শোপিস": "Showpieces",
  "ফ্রেম টেরারিয়াম": "Frame terrarium",
  "স্নেক টেরারিয়াম": "Snake-plant terrarium",
  "হার্ব টেরারিয়াম": "Herb terrarium",
  "জার কাস্টমাইজ": "Customize jar",
  "ফ্রেম রং": "Frame color",
  "কাচের রং": "Glass tint",
  "চওড়া": "Width",
  "উচ্চতা": "Height",
  "আইটেম সাজাও": "Adjust item",
  "বসানো আইটেম বেছে নাও": "Choose a placed item",
  "আইটেম বেছে নাও": "Select an item",
  "আগে সাজানো থেকে একটি আইটেম বসাও।": "Place an item from Decorations first.",
  "জারের আইটেমে ট্যাপ করো।": "Tap an item in the jar.",
  "নিজের রং": "Custom colour",
  "সাইজ": "Size",
  "ঘোরাও": "Rotate",
  "রং": "Color",
  "মুছে ফেলো": "Delete",
  // jar-mounted lamp
  "আলো": "Light",
  "আলোর উচ্চতা": "Light height",
  "উজ্জ্বলতা": "Brightness",
  "দরজা": "Door",
  "ডিফল্ট": "Default",
  "খোলা": "Open",
  "চালু": "On",
  "বন্ধ": "Off",
  // staging tray
  "ট্রে": "Tray",
  "ট্রে থেকে বানাও": "Build from tray",
  "ট্রেতে যোগ করা হয়েছে": "Added to tray",
  "আগে ট্রেতে আইটেম যোগ করো (＋)": "Add items to the tray first (＋)",
  "＋ দিয়ে ট্রেতে যোগ করো": "Use ＋ to add items to the Tray",
  "কার্ডে ট্যাপ করে সরাসরি বসাও": "Tap a card to place it directly",
  "ট্রে খুলে তালিকা থেকে বানাও": "Open Tray to build from your list",
  "ট্রে থেকে বেছে বসাও, পানি ঢালো, স্প্রে করো।":
    "Pick from your tray, place items, pour water, and mist.",
  // folding the left column away
  "সাইডবার লুকাও": "Hide sidebar",
  "সাইডবার দেখাও": "Show sidebar",

  // --- species pack --------------------------------------------------------
  // new categories
  "মস": "Moss",
  "জলজ গাছ": "Aquatic plants",
  // named mosses
  "কুশন মস": "Cushion moss",
  "স্টার মস": "Star moss",
  "ক্যাথরিন মস": "Catherine's moss",
  "ফিসিডেন্স মস": "Fissidens moss",
  "ফার্ন মস": "Fern moss",
  "ব্রুম ফর্ক মস": "Broom fork moss",
  "জাভা মস": "Java moss",
  "ট্র্যাকিসিস্টিস মস": "Trachycystis moss",
  // aquarium genera
  "রোটালা": "Rotala",
  "লুডউইজিয়া": "Ludwigia",
  "বাকোপা": "Bacopa",
  "হাইগ্রোফিলা": "Hygrophila",
  "পার্সিকেরিয়া": "Persicaria",
  "লিন্ডারনিয়া": "Lindernia",
  "অল্টারনানথেরা": "Alternanthera",
  "ক্যাবোম্বা": "Cabomba",
  "মিরিওফাইলাম": "Myriophyllum",
  "অ্যাম্বুলিয়া": "Ambulia",
  "অ্যানাকারিস": "Anacharis",
  "পোগোস্টেমন": "Pogostemon",
  "সোর্ড লিলি": "Sword lily",
  "ক্রিপ্টোকোরিন": "Cryptocoryne",
  "ডোয়ার্ফ স্যাজিটেরিয়া": "Dwarf sagittaria",
  "ওয়াটার উইস্টেরিয়া": "Water wisteria",
  "হাইড্রোকোটাইল": "Hydrocotyle",
  "ওয়াটার পপি": "Water poppy",
  // aquarium species / cultivars
  "বাংলাদেশ": "Bangladesh",
  "ভিয়েতনাম": "Vietnam",
  "বাটারফ্লাই": "Butterfly",
  "অরেঞ্জ জুস": "Orange juice",
  "রোটান্ডিফোলিয়া": "Rotundifolia",
  "পেরুয়েনসিস": "Peruensis",
  "পালুস্ট্রিস": "Palustris",
  "ডায়মন্ড": "Diamond",
  "মেটা": "Meta",
  "সুপার রেড": "Super red",
  "মনিয়েরি": "Monnieri",
  "ক্যারোলিনিয়ানা": "Caroliniana",
  "অ্যামপ্লেক্সিকলিস": "Amplexicaulis",
  "করিম্বোসা": "Corymbosa",
  "অ্যাঙ্গুস্টিফোলিয়া": "Angustifolia",
  "গ্রিন অলিভ": "Green olive",
  "সাও পাওলো": "Sao Paulo",
  "রেইনেকি": "Reineckii",
  "অ্যাকোয়াটিকাম": "Aquaticum",
  "ডেনসা": "Densa",
  "স্টেলাটাস": "Stellatus",
  "অ্যারোমেটিক মিনি": "Aromatic mini",
  "ট্রাইপার্টিটা": "Tripartita",
  "মার্বেল কুইন": "Marble queen",
  // aroids and the rest of the bench
  "অ্যাগ্লোনিমা": "Aglaonema",
  "অ্যান্থুরিয়াম": "Anthurium",
  "অ্যালোকেসিয়া": "Alocasia",
  "ফিলোডেনড্রন": "Philodendron",
  "হাওরথিয়া": "Haworthia",
  "আইভি": "Ivy",
  "ক্লারিনার্ভিয়াম": "Clarinervium",
  "ক্রিস্টালিনাম": "Crystallinum",
  "পলি": "Polly",
  "সিলভার ড্রাগন": "Silver dragon",
  "ব্ল্যাক ভেলভেট": "Black velvet",
  "জ্যানাডু": "Xanadu",
  "সাদা ছোপ": "Speckled",
  "ভ্যারিগেটেড": "Variegated",
  // clean-up crew
  "নেরাইট শামুক": "Nerite snail",
  "চেরি চিংড়ি": "Cherry shrimp",
  "টাইগার": "Tiger",
  "জেব্রা": "Zebra",

  // --- closed-terrarium plants and the clean-up crew ------------------------
  "স্পাইক মস": "Spikemoss",
  "পেপেরোমিয়া": "Peperomia",
  "বেবি টিয়ার্স": "Baby tears",
  "ক্রিপিং ফিগ": "Creeping fig",
  "আইসোপড": "Isopod",
  "স্প্রিংটেইল": "Springtails",
  "ওয়াটারমেলন": "Watermelon",
  "ডেইরি কাউ": "Dairy cow",
  "সোনালি": "Golden",
  // where a plant actually wants to live
  "বদ্ধ জারে": "Closed jar",
  "খোলা পাত্রে": "Open vessel",
  "জলজ": "Aquatic",
  "আর্দ্র, বদ্ধ জারে ভালো থাকে": "Thrives sealed and humid",
  "বাতাস চলাচল আর শুকনো সময় দরকার": "Needs airflow and a dry spell",
  "পানিতে বা ভেজা তীরে বাঁচে": "Lives in water or on a wet bank",
  // mould
  "মোল্ড ছড়াচ্ছে — কম পানি দাও, আর স্প্রিংটেইল বা আইসোপড যোগ করো।":
    "Mould is spreading — water less, and add springtails or isopods.",
  "মোল্ড ছড়াচ্ছে — কিছুদিন পানি কম দাও, দলটাকে কাজ করতে দাও।":
    "Mould is spreading — ease off the water and let the crew work.",

  // --- printed hardscape set -----------------------------------------------
  "মাশরুম সেতু": "Mushroom bridge",
  "দড়ির সেতু": "Rope bridge",
  "হেলানো কুটির": "Crooked cottage",
  "টিউডর ঘর": "Tudor house",
  "শঙ্খ ঘর": "Shell house",
  "কাঠের কেবিন": "Log cabin",
  "মাশরুম কুটির": "Toadstool cottage",
  "গম্বুজ কুটির": "Dome cottage",
  "ডাইনির টুপি ঘর": "Witch hat house",
  "প্যাঁচানো টাওয়ার": "Spiral tower",
  "গির্জা": "Chapel",
  "গুঁড়ি ঘর": "Stump house",
  "ধাপ মন্দির": "Stepped ruin",
  "পাথুরে গুহা": "Rock cave",
  "স্লেট ধাপ": "Slate ledges",
  "ক্যানিয়ন": "Canyon",
  "পাথুরে সিঁড়ি": "Stone stairs",
  "ভাঙা দেয়াল": "Broken wall",
  "ভাঙা টাওয়ার": "Ruined tower",
  "চীনা মন্দির": "Temple hall",
  "মণ্ডপ": "Pavilion",
  "নোঙর": "Anchor",
  "ম্যান্টিস": "Mantis",
  "সাপ": "Snake",
  "নেকড়ে": "Wolf",
  "আইবেক্স": "Ibex",
  "হাতি": "Elephant",
  // their variant suffixes
  "সোজা": "Flat",
  "খিলান": "Arched",
  "কাঠের": "Timber",
  "কুটির": "Cottage",
  "দোতলা": "Two storey",
  "একতলা": "Single storey",
  "সরাইখানা": "Tavern",
  "প্রাচীরঘেরা": "Walled",
  "ছোট": "Small",
  "দোতলা ছাউনি": "Double eave",
  "পাথরের ভিত": "Stone podium",
  "লাল ছাদ": "Red cap",
  "বেলেপাথর": "Sandstone",
  "শ্যাওলা": "Mossy",
  "বালি": "Sand",
  "পাথুরে": "Stone",

  // --- the flowering garden pack -------------------------------------------
  "ফুলগাছ": "Flowers",
  "রেইন লিলি": "Rain lily",
  "কাঁটামুকুট": "Crown of thorns",
  "রুয়েলিয়া": "Ruellia",
  "অ্যামারিলিস": "Amaryllis",
  "অপরাজিতা": "Butterfly pea",
  "ল্যান্টানা": "Lantana",
  "ওয়েডেলিয়া": "Creeping daisy",
  "কলিয়াস": "Coleus",
  "ড্রিমিওপসিস": "Drimiopsis",
  "ক্যালানচো": "Kalanchoe",
  "অক্সালিস": "Purple shamrock",
  // rain lily cultivars
  "আইভরি": "Ivory",
  "সানশাইন": "Sunshine",
  "কিং র‍্যানসম": "King Ransom",
  "লামদুয়ান রেড": "Lamduan Red",
  "রেড জয়": "Red Joy",
  "রাস্টিক রে": "Rustic Ray",
  "পিংক ফেয়ারি": "Pink Fairy",
  "সুইট হার্ট": "Sweet Heart",
  "হার্ট থ্রব": "Heart Throb",
  "জোডিয়াক সারপ্রাইজ": "Zodiac Surprise",
  "ক্যাপ্রিকর্ন": "Capricorn",
  "লাবুফেরোসা": "Labuferossa",
  "স্মল হাইব্রিড": "Small hybrid",
  // the other variant suffixes in the pack
  "কাঁটাবিহীন লাল": "Thornless red",
  "হরিদ্রা মুকুট": "Haridra",
  "ফাল্গুনী": "Falguni",
  "সাঁঝবাতি মুকুট": "Sanjhbati",
  "পিচ": "Peach",
  "মেজেন্টা": "Magenta",
  "ছোপ পাতা": "Spotted leaf",
  "আমব্রেলাটা": "Umbellata",

  // --- printed miniatures, second wave -------------------------------------
  "ডাল বেঞ্চ": "Branch bench",
  "ঢাকা গাড়ি": "Covered wagon",
  "চাকা কুয়া": "Winch well",
  "প্যাগোডা টাওয়ার": "Pagoda tower",
  "বারান্দা কুটির": "Porch cottage",
  "পরি টাওয়ার": "Fairy tower",
  "শ্যান্টারেল": "Chanterelle",
  "টেরাকোটা": "Terracotta",
  "ক্যানভাস": "Canvas",
  "ধূসর ছাদ": "Grey roof",
  "জাপানি": "Japanese",

  // --- open vessels --------------------------------------------------------
  "টেরাকোটা টব": "Terracotta pot",
  "ডিশ গার্ডেন": "Dish garden",
  "পাথরের আর্ন": "Stone urn",
  "কাঠের ট্রাফ": "Wooden trough",
  "খোলা পাত্র: বাতাস চলে, বদ্ধ হয় না": "Open vessel: air moves, nothing seals in",
};

const LANG_KEY = "potroneer-lang";
let lang = localStorage.getItem(LANG_KEY) || localStorage.getItem("terrarium-lang") || "bn";

export function getLang() {
  return lang;
}

export function setLang(l) {
  lang = l;
  localStorage.setItem(LANG_KEY, l);
}

// Translate a single string.
export function t(bn) {
  if (lang === "bn") return bn;
  return EN[bn] ?? bn;
}

// Translate a composite label like "মস · সবুজ" part by part.
export function tLabel(label) {
  if (lang === "bn") return label;
  return label
    .split(" · ")
    .map((part) => EN[part] ?? part)
    .join(" · ");
}
